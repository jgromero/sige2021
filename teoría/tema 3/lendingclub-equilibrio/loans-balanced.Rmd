---
title: "Equilibrio de clases con Lending Club"
...
Partiendo de la selección de variables realizada en un [ejercicio anterior](https://github.com/jgromero/sige2021/blob/main/pr%C3%A1cticas/selecci%C3%B3n%20de%20variables/lending%20club/lending_club_seleccion.Rmd), construiremos vamos modelos de predicción aplicando varias técnicas de balanceo (o reequilibrado) de clases.
...
Leemos el fichero de datos generado previamente, con la selección de variables ya hecha. Eliminamos NAs para lo que resta del cuaderno. Para simplificar el proceso, seleccionamos el 5% de los datos para entrenamiento+validación y reservamos el 20% para test.
...
data <- data_raw %>%
  top_n(round(0.05 * nrow(data_raw))) %>%
  na.exclude() %>%
  mutate(loan_status = as.factor(loan_status))
data
```

Atención: Esta validación está hecha con el conjunto obtenido después del "downsampling". Para hacer la comparación adecuadamente, hacemos una validación con un conjunto de datos adecuado (aquí se hace con los datos de test reservados previamente).

```{r}
# Selección de datos para test
test <- data_raw %>%
  top_n(-round(0.2 * nrow(data_raw))) %>%
  na.exclude() %>%
  mutate(loan_status = as.factor(loan_status))
```
...
# Crear modelo de predicción con partición aleatoria
...
prediction_p_test <- predict(rfModel, test, type = "prob")
result <- my_roc(test, prediction_p_test, "loan_status", "Paid")
...
# Crear modelo de predicción con "downsampling"
...
También se puede visualizar el resultado del test.
...
# Crear modelo de predicción con "upsampling"
...
# Crear modelo de predicción con SMOTE

Por último, hacemos rebalanceado de clases utilizando:

-   [<tt>SMOTE</tt>](https://www.rdocumentation.org/packages/DMwR/versions/0.4.1/topics/SMOTE) de [<tt>DMwR</tt>](https://www.rdocumentation.org/packages/DMwR/versions/0.4.1)

-   [<tt>SMOTE</tt>](https://www.rdocumentation.org/packages/smotefamily/versions/1.3.1/topics/SMOTE) de [<tt>smotefamily</tt>](https://www.rdocumentation.org/packages/smotefamily/versions/1.3.1)
...
# DMwR: data_smote <- SMOTE(loan_status ~ ., data_for_SMOTE, perc.over = 600, dataperc.under = 100)  
smote <- SMOTE(data_for_SMOTE[,-1], target = data_for_SMOTE[,1], K = 10, dup_size = 6)
data_smote <- smote$data %>%
  rename(loan_status = class) %>%
  mutate(loan_status = ifelse(loan_status == 1, "Paid", "Unpaid"))
  
table(data_smote$loan_status) 

# Crear particiones aleatorias sobre el dataset ampliado con SMOTE
trainIndex <- createDataPartition(data_smote$loan_status, p = .75, list = FALSE, times = 1)
train <- data_smote[ trainIndex, ] 
val   <- data_smote[-trainIndex, ]
print("Datos de entrenamiento después de SMOTE: ")
table(train$loan_status)
print("Datos de validación después de SMOTE: ")
table(val$loan_status)
...
prediction_p <- predict(rfModel, val_num, type = "prob")
prediction_r <- predict(rfModel, val_num, type = "raw")
result <- my_roc(val, prediction_p, "loan_status", "Paid")
...
prediction_p_test <- predict(rfModel, test_num, type = "prob")
result <- my_roc(test_num, prediction_p_test, "loan_status", "Paid")
...
# Evaluar modelos sobre el conjunto de test
...
results <- c(list(orig_roc), list(down_roc), list(up_roc), list(smote_roc))
...
# Modelo con "smote" : cambiamos el tipo de sampling en rfCtrl a "smote" (necesita DMwR)
